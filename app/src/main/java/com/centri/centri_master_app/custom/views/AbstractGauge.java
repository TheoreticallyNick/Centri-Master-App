...
abstract class AbstractGauge extends View {
    ...
    private List<com.centri.centri_master_app.custom.views.Range> ranges = new ArrayList<>();
    ...
    public void addRange(com.centri.centri_master_app.custom.views.Range range) {
        if (range == null)
            return;
        ranges.add(range);
    }
    ...
    public List<com.centri.centri_master_app.custom.views.Range> getRanges() {
        return ranges;
    }

    public void setRanges(List<com.centri.centri_master_app.custom.views.Range> ranges) {
        this.ranges = ranges;
    }
    ...
    protected int getRangeColorForValue(double value, List<com.centri.centri_master_app.custom.views.Range> ranges) {
        int color = Color.GRAY;

        for (com.centri.centri_master_app.custom.views.Range range : ranges) {
    ...
    protected int getCalculateValuePercentage(double min, double max, double value) {
        if (min < 0 && max < 0 && min < max) {
            ...
            if (min > max) {
                ...
                return getCalculateValuePercentageUseCaseFour(min, max, value);
            }
    ...
    private int getCalculateValuePercentageUseCaseThree(double min, double max, double value) {
        ...
        if (value <= Math.min(min, max)) {
            ...
            double result = ((positive - value) / (available) * 100);
            return (int) result;
        } else {
     ...
     * TODO: Need Improvements
     */
    private int getCalculateValuePercentageUseCaseFour(double min, double max, double value) {
        double available = Math.abs(min) + Math.abs(max);
        if (value <= Math.min(min, max)) {
            return 0;
        } else if (value >= Math.max(min, max))
